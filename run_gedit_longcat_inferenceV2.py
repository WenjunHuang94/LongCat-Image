...
def main():
    ...
    # === ✨ 修改：按任务类别 (task_type) 均匀抽取指定数量的图片 ===
    if args.num_images is not None and args.num_images > 0:
        print(f"🛠️ Debug Mode: 正在从每个任务类别中随机抽取 {args.num_images} 张图片...")
        # 1. 先全局打乱，保证每次抽取的随机性
        shuffled_data = data_split.shuffle(seed=args.seed)

        from collections import defaultdict
        task_counts = defaultdict(int)
        selected_indices = []

        # 2. 遍历数据集，收集每个类别的前 N 张图的索引
        ...
        # 3. 根据收集到的索引生成新的子数据集
        data_split = shuffled_data.select(selected_indices)

        print(f"✅ 抽样完成！总共提取了 {len(data_split)} 张测试图。")
        print(f"📊 数据分布: {dict(task_counts)}")
